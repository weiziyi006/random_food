...
def get_draw_records(
        userid: int,
        limit: int = Query(10, description="查询记录条数，默认10条")
):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
        select top (?) 
            category_name, shop_name, address
        from food_draw_record
        where userid = ?
...
def get_draw_records(userid: int, limit: int = 15):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            select id, category_name, shop_name, address from food_draw_record
...
def get_shop_rank(limit: int = 15):
    if limit <= 0 or limit > 20:
        return {"code": 400, "msg": "limit 必须是 1-20 之间的整数"}
    conn = None
    cursor = None
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("""
            select shop_name,category_name,address,score from food_list
            group by shop_name, category_name, address, score
            order by score desc, shop_name collate Chinese_PRC_CS_AS_KS_WS asc
            offset 0 rows fetch next ? rows only;
        """, (limit,))
        columns = [d[0] for d in cursor.description]
        rank_list = []
        for row in cursor.fetchall():
            record = dict(zip(columns, row))
            record["score"] = round(float(record["score"]), 1)
            rank_list.append(record)
        return {"code": 200, "msg": "success", "data": rank_list}
    except Exception as e:
        return {"code": 500, "msg": f"服务器错误：{str(e)}"}
    finally:
        if cursor:
...
def smart_recommend(
    ...
    city_adcode: str = Query("130900")  # 默认沧州
):
    ...
    try:
        ...
        sql = """
        select 
            f.shopid, f.shop_name, f.category_name, f.address, f.score,
        ...
        group by f.shopid, f.shop_name, f.category_name, f.address, f.score, uc.cnt
        """
        cursor.execute(sql)
        rows = cursor.fetchall()
        if not rows:
